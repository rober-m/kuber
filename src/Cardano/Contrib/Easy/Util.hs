...
signAndSubmitTxBody conn txBody skeys= do
  let tx = makeSignedTransaction (map toWitness skeys) txBody -- witness and txBody
...
calculateTxoutMinLovelace txout pParams=do
  ...
  Just $ Lovelace  $ Alonzo.utxoEntrySize (toShelleyTxOut ShelleyBasedEraBabbage  txout) * costPerWord
...
calculateTxoutMinLovelaceFunc pParams = do
  ...
  pure $ f costPerWord
  where
    f cpw txout =Lovelace  $ Alonzo.utxoEntrySize (toShelleyTxOut ShelleyBasedEraBabbage  $  toCtxUTxOTxOut txout) * cpw
